...
async function packExternalModules() {
    ...
    const fallbackPackagePath = upperPackageJson && path_1.default.relative(process.cwd(), path_1.default.join(upperPackageJson, './package.json'));
    // eslint-disable-next-line no-nested-ternary
    const packageJsonPaths = configuredPackagePaths
        ? (0, helper_1.asArray)(configuredPackagePaths)
        : fallbackPackagePath
            ? [fallbackPackagePath]
            : [];
    const uniquePackageJsonPaths = R.uniq(packageJsonPaths.filter((value) => Boolean(value)));
    (0, assert_1.default)(uniquePackageJsonPaths.length > 0, 'packageJsonPath is not defined');
    ...
    const externalModulesByPath = uniquePackageJsonPaths.map((packageJsonPath) => {
        const modules = Array.from(new Set(getProdModules.call(this, externalModules, packageJsonPath, rootPackageJsonPath)));
        return {
            packageJsonPath,
            modules,
        };
    });
    const compositeModules = Array.from(new Set(externalModulesByPath.reduce((acc, { modules }) => {
        acc.push(...modules);
        return acc;
    }, [])));
    ...
    const packageLockPath = path_1.default.join(process.cwd(), packager.lockfileName);
